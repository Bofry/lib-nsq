...
type mockMessageState map[string][]byte
...
func (s mockMessageState) Del(name string) (old []byte) {
	old, _ = s[name]
	delete(s, name)
	return
}
func (s mockMessageState) Set(name string, value []byte) (old []byte, err error) {
	old, _ = s[name]
	s[name] = value
	return
}
func (s mockMessageState) Value(name string) []byte {
	return s[name]
}
func (s mockMessageState) Visit(visit func(name string, value []byte)) {
	for k, v := range s {
		visit(k, v)
	}
}
